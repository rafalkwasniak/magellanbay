...
{{-- Okruszki (breadcrumb) storefrontu. `items` = lista pozycji od korzenia do
     ...
     przez klasy st-*. `back` (opcjonalny URL) = krótki „← Powrót" doklejony w
     PRAWO w tej samej linii co ścieżka — na podstronach wymagających powrotu
     (np. produkt → wykaz). Poza widoczną ścieżką dokłada schema.org
     BreadcrumbList (JSON-LD), z którego Google renderuje okruszki w wynikach. --}}
@php $items = array_values(array_filter($items)); @endphp

@if (count($items) > 1 || $back)
    @php $last = count($items) - 1; @endphp
    <div class="flex items-center justify-between gap-4">
        <nav aria-label="Ścieżka nawigacji" class="min-w-0 text-sm">
        ...
        @if ($back)
            <a href="{{ $back }}" class="shrink-0 text-sm opacity-70 transition hover:opacity-100">← Powrót</a>
        @endif
    </div>
